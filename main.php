...
function makeApiRequest($userId, $tgId, $proxy) {
...
function claimReward(&$totalPoints, $rewardUrl, $headers) {
    $ch = curl_init();
...
function addUser($users, $userId, $tgId) {
    $users[$userId] = [
        'tg_id' => $tgId
    ];
...
$proxies = file_exists($proxiesFile) ? file($proxiesFile, FILE_IGNORE_NEW_LINES) : [];
...
while (true) {
    ...
    foreach ($users as $userId => $userData) {
        ...
        list($response, $httpCode, $reqHeaders) = makeApiRequest($userId, $tgId, $proxy);

        if ($httpCode === 200) {
            $reward = extractReward($response);
            if ($reward) {
                // Claim reward (add PX points)
                if (claimReward($totalPoints, $reward, $reqHeaders)) {
